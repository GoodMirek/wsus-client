...
action :install do
  ...
  converge_by "installing #{update_collection.count} update(s)" do
    # Transforms to a new update collection
    update_collection = WIN32OLE.new('Microsoft.Update.UpdateColl')
    ...
    if result.RebootRequired && new_ressource.on_reboot_required
      recipe_eval &new_ressource.on_reboot_required
    end
  end
end
...
def assert_result(action, result)
  if result.HResult != 0 || result.ResultCode != SUCCESS_CODE
